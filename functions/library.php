...
class library{
	...
	/*------------- Hàm select list ----------------*/
	    ...
	    public function getList($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
	     ...
	     public function getList_1($table, $name_idCat, $id_Cat, $order_name, $order_state, $trang, $limit, $page, $slug = ''){
...
//            $results = $stmt->get_result();
	...
	/*--------------- Danh mục đệ quy ---------------*/
	        	...
	        	foreach ($cate_child as $key => $item) {
	        		...
	        		echo '<tr>';
		                ...
		                echo '<td>';
		                    ...
		                    echo '<a href="index.php?page='.$page.'&id='.$item[$name_id].'&khoa_id='.$_GET['khoa_id'].'" title="" style="padding-left: '.($level*20).'px">'.$item[$name_text].'</a>';
		...
		function getTypePage_byUrl($urlFriendly,$lang){
		    ...
		    // $where = "where $nameColTypeLanguage = '".$lang."' AND $nameColUrlFriendly = '".$urlFriendly."'";
		    // $sql = "SELECT * from $table $where";echo $sql;
