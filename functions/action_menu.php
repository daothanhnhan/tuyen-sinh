...
class action_menu extends library{
    ...
    public function setUrlFriendly_byType($id_menu,$lang){
        ...
        switch ($rowMenu[$this->nameColTypeId_menu]) {
            ...
            case '5': //danh muc tin tuc
                $rowItemLanguage = $this->getDetailItemLanguage_byId('newscat',$this->nameColIdNewsCat_newsCatLanguage,$rowMenu[$this->nameColNewsCatId_menu],$this->nameColType_newsCatLanguage,$lang);
                $url = '/'.$rowItemLanguage[$this->nameColUrlNewsCat_newsCatLanguage]; 
                break;          
            case '7': //bai viet tin tuc
                $rowItemLanguage = $this->getDetailItemLanguage_byId('news' ,$this->nameColIdNews_newsLanguage,$rowMenu[$this->nameColNewsId_menu],$this->nameColType_newsLanguage,$lang);
                $url = '/'.$rowItemLanguage[$this->nameColUrlNews_newsLanguage];                
                break;

            case '8': // dia chi web
                $url = '/'.$rowMenu['menu_url'];
                break; 
            ...
            case '9': // liên hệ
                ...
                break; 
            case '13': // page
                $rowItemLanguage = $this->getDetailItemLanguage_byId('page' ,$this->nameColId_pageLanguage,$rowMenu[$this->nameColPageId_menu],$this->nameColType_pageLanguage,$lang);
                $url = '/'.$rowItemLanguage['friendly_url']; 
                break;
    ...
    public function setUrlFriendly_byType_1($id_menu,$lang){
        global $conn_vn;
        //$url = '';
    ...
    public function getDetailItemLanguage_byId($nameTable_item,$nameColId_item,$valId_item,$nameColTypelang_item,$lang){
        ...
        $sql = "SELECT * from $nameTable_item where ($nameColId_item = '".$valId_item."') limit 1";//echo $sql;
    ...
    public function getDetailItemLanguage_byId_1($nameTable_item,$nameColId_item,$valId_item,$nameColTypelang_item,$lang){
    ...
    public function show_menu1 ($list, $lang) {
        ...
        foreach ($list as $item) {
            ...
            if ($url1 == '/lien-he') {
                $url1 = '/lien-he/'.$khoa_id;
            }
            if ($url1 == '/nhan-su') {
                $url1 = '/nhan-su/'.$khoa_id;
            }
    ...
    public function show_menu1_1 ($list, $lang) {
        global $khoa_id;

        // $menu1 = new action_menu();
        echo '<ul class="list_main_menu_1">';
            $khoa_menu = 1;
            if ($khoa_id == 0) {$khoa_menu = 0;}
            if ($khoa_id == 1) {$khoa_menu = 0;}
            if ($khoa_id == 2) {$khoa_menu = 0;}
            if ($khoa_id == 3) {$khoa_menu = 0;}

            if ($khoa_menu == 1) {
                $action = new action();
                $khoa = $action->getDetail('khoa', 'id', $khoa_id);
                echo '<li class="item_main_menu_1"><a href="/khoa/'.$khoa_id.'" class="link_main_menu_1" style="background:red;">'.$khoa['name'].'</a></li>';
            }
        foreach ($list as $item) {
            $url1 = $this->setUrlFriendly_byType($item['menu_id'], $lang);
            echo '<li class="item_main_menu_1">';
    ...
    public function show_menu_sub_mb ($list, $lang) {
        global $khoa_id;
        ...
        // $menu1 = new action_menu();
            ...
            if ($khoa_menu == 1) {
                ...
                echo '<option value="/khoa/'.$khoa_id.'">'.$khoa['name'].'</option>';
            }
        foreach ($list as $item) {
            $url1 = $this->setUrlFriendly_byType($item['menu_id'], $lang);
            ...
            echo '<option value="'.$url1.'">'.$item['lang_menu_name'].'</option>';
            // echo '<a href="http://alibabatinhgia.webmienphi.org.vn" class="link_main_menu_1">'.$item['lang_menu_name'].'2</a>';
            $list_sub = $this->main_menu($item['menu_id'], $lang);
            if (count($list_sub) != 0) {
                $this->show_menu_sub_mb_2($list_sub, $lang);
            }
    ...
    public function show_menu_sub_mb_2 ($list, $lang) {
        global $khoa_id;
