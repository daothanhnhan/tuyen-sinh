...
public function getNewsCatDetail_byUrl($url){
    global $conn_vn;

    $table = $this->nameTable_newsCat;

    $where = "where friendly_url = '".$url."'";
...
public function getNewsCatLangDetail_byUrl_1($urlFriendly,$lang){

    global $conn_vn;

    $sql = "SELECT * FROM newscat WHERE friendly_url = '$urlFriendly'";
    $result = mysqli_query($conn_vn, $sql);
    $row = mysqli_fetch_assoc($result);
    ...
    $where = "where newscat_id = $newscat_id and $this->nameColType_newsCatLanguage = '".$lang."'";
...
public function getNewsLangDetail_byUrl_1($urlFriendly,$lang){
    global $conn_vn;

    $sql = "SELECT * FROM news WHERE friendly_url = '$urlFriendly'";
    $result = mysqli_query($conn_vn, $sql);
    $row = mysqli_fetch_assoc($result);
    $news_id = $row['news_id'];

    $table = $this->nameTable_newsLanguage;
    $where = "where news_id = $news_id and $this->nameColType_newsLanguage = '".$lang."'";
    $limit = "limit 1";
    $sql = "SELECT * from $table $where $limit";        
    $result = mysqli_query($conn_vn,$sql);
    if (mysqli_num_rows($result) > 0){
...
public function getNewsList_byMultiLevel_orderNewsId_search($idNewsCat,$order_newsId,$trang, $limit, $page){
    ...
    if (isset($_POST['q'])) {
        if (!empty($_POST['q'])) {
            $q_arr = explode(" ", $_POST['q']);
            $where .= " AND (1=1 ";
            foreach ($q_arr as $i) {
                $i = vi_en1($i);
                $where .= " AND friendly_url LIKE '%$i%'";
            }
            $where .= ")";
            $cond = '/'.vi_en1($_POST['q']);
        } else {
    ...
    if ($trang != "" && $limit != "") {
        ...
        $config = array(
            ...
            'link_full'=> $slug != '' ? $slug.'/{trang}'.$cond : '/'.$page.'/{trang}'.$cond,

            'link_first'=> $slug != '' ? $slug : '/'.$page.'/1'.$cond,
...
public function getListNewsNew_hasLimit($limit){
    ...
    $limit = "limit ".$limit;
    // $sql = "SELECT * from $table $where $limit";  
    $sql = "SELECT * from $table order by news_id desc $limit";   // echo $sql;
