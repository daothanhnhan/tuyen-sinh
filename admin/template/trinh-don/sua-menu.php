<?php 
    ...
    $list = $action->getList('menu','khoa_id',$row['khoa_id'],'menu_id','asc','','','');
...
<script>
    function getMenuType(val){
        // alert($(val).val());
        $.ajax({
            url: 'ajax.php?action=getMenuType&khoa_id=<?= $_GET['khoa_id'] ?>&menu='+$(val).val(),
            type: 'get',
            ...
            <div class="content">
                <?php 
                    switch ($row['menu_type_id']) {
                        ...
                        case '5':
                            $list = $action->getList('newscat','khoa_id',$_GET['khoa_id'],'newscat_id','asc','','','');
                            ?>
                        ...
                        case '7':
                            $list = $action->getList('news','khoa_id',$_GET['khoa_id'],'news_id','asc','','','');
                            ?>
                       ...
                       case '13':
                            $list = $action->getList('page','khoa_id',$_GET['khoa_id'],'page_id','asc','','','');
                            ?>
                                <p class="titleRightNCP">Trang nội dung</p>
                        ...
                        case '17':
                            $list = $action->getList('khoa','','','id','asc','','','');
                            ?>
                                ...
                                <select name="khoa_menu_id" id="" class="select2">
                                    <?php 
                                        foreach ($list as $key => $value) {
                                        ?>
                                            <option value="<?= $value['id']?>" <?= $row['khoa_menu_id'] == $value['id'] ? 'selected' : ''?> ><?= $value['name']?></option>
                                        <?php
                                        }
                                    ?>
                                </select>
