...
class action_page extends library{
	...
	/*-------------- Cấu hình trang chủ --------------------*/
		...
		public function updateConfig(){
	    	...
	    	if (isset($_FILES['fileUpload6']) && $_FILES['fileUpload6']['name'] != "") {

	    		$src= "../images/";

		    	$img = $_FILES['fileUpload6'];
	...
	/*--------------- Danh mục sản phẩm -------------------*/
		...
		public function deleteProductCat(){
			...
			if ($productcat_id == 139 || $productcat_id == 140 || $productcat_id == 124 || $productcat_id == 151) {
	...
	/*--------------- Sản phẩm --------------------*/
		...
		public function addProduct(){
			...
			$product_favorite = isset($_POST['product_favorite']) ? $_POST['product_favorite'] : '';
   ...
   //          }
	  ...
	  //   	if(isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != ""){
			...
			$insert = $this->Insert_1('product', 
									   ...
									   'sssssssdddsssisiiissssssssssssssssissssii');
   ...
   //          }
	...
	/*----------------- Trang nội dung -------------------*/
		...
		public function addPage(){
	    	...
	    	if($insert != ''){
	    		...
	    		$friendly_url_2 = $friendly_url.'-page'.$insert;
	    		$sql = "UPDATE page SET friendly_url = '$friendly_url_2' WHERE page_id = $insert";
	    		$result = mysqli_query($conn_vn, $sql);
	    		...
	    		foreach ($languages as $key => $lang) {
	    			...
	    			if ($lang['languages_code'] == 'vn') {
	    				$friendly_url = $friendly_url_2;
	    			}
	    		...
	    		echo json_encode(array('success'=>'Thêm mới thành công', 'url'=>'index.php?page=sua-bai-viet&id='.$insert.'&khoa_id='.$khoa_id));
		...
		public function deletePage(){
			...
			$row = $this->getDetail_New('page',array('page_id'), array(&$page_id),'i');
			if($this->Delete_1('page','',array(&$page_id),array('page_id'),'i')){

				echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=bai-viet&khoa_id='.$row['khoa_id']));
	...
	/*---------------- Danh mục tin tức --------------------*/
		...
		public function addNewsCat(){
			...
			$dai_hoc = isset($_POST['dai_hoc']) ? $_POST['dai_hoc'] : '0';
			$thac_si = isset($_POST['thac_si']) ? $_POST['thac_si'] : '0';
			$tien_si = isset($_POST['tien_si']) ? $_POST['tien_si'] : '0';
			$tin_tuc = isset($_POST['tin_tuc']) ? $_POST['tin_tuc'] : '0';
			$su_kien = isset($_POST['su_kien']) ? $_POST['su_kien'] : '0';
	    	...
	    	$insert = $this->Insert_1('newscat', 
	    							   ...
	    							   'sssiissssssiiiiiiiisssssi');

	    	

	    	if($insert != ''){

	    		$friendly_url_2 = $friendly_url.'-cat'.$insert;
	    		$sql = "UPDATE newscat SET friendly_url = '$friendly_url_2' WHERE newscat_id = $insert";
	    		$result = mysqli_query($conn_vn, $sql);
	    		...
	    		echo json_encode(array('success'=>'Thêm mới thành công', 'url'=>'index.php?page=sua-danh-muc-tin-tuc&id='.$insert.'&khoa_id='.$khoa_id));
		...
		public function updateNewsCat(){
	    	...
	    	$arr_url = explode("-", $friendly_url);
	    	$end_arr_url = end($arr_url);
	    	if ($end_arr_url != 'cat'.$newscat_id) {
	    		$friendly_url = $friendly_url.'-cat'.$newscat_id;
	    	}
		...
		public function deleteNewsCat(){
			...
			$row = $this->getDetail_New('newscat', array('newscat_id'), array(&$newscat_id), 'i');
			if($this->Delete_1('newscat','',array(&$newscat_id),array('newscat_id'),'i')){

				echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=danh-muc-tin-tuc&khoa_id='.$row['khoa_id']));
	...
	/*------------------ Tin tức -------------------*/
		...
		public function addNews(){
			...
			$de_tai_du_an = isset($_POST['de_tai_du_an']) ? $_POST['de_tai_du_an'] : '0';
			$cong_bo_khoa_hoc = isset($_POST['cong_bo_khoa_hoc']) ? $_POST['cong_bo_khoa_hoc'] : '0';
			$co_so_vat_chat = isset($_POST['co_so_vat_chat']) ? $_POST['co_so_vat_chat'] : '0';
	    	...
	    	$insert = $this->Insert_1('news', 
	    							   ...
	    							   'ssssssiissssiiiiisssssi');



	    	if($insert != ''){

	    		$friendly_url_2 = $friendly_url.'-p'.$insert;
	    		$sql = "UPDATE news SET friendly_url = '$friendly_url_2' WHERE news_id = $insert";
	    		$result = mysqli_query($conn_vn, $sql);
	    		...
	    		echo json_encode(array('success'=>'Thêm mới thành công', 'url'=>'index.php?page=sua-tin-tuc&id='.$insert.'&khoa_id='.$khoa_id));
		...
		public function updateNews(){
	    	...
	    	$arr_url = explode("-", $friendly_url);
	    	$end_arr_url = end($arr_url);
	    	if ($end_arr_url != 'p'.$news_id) {
	    		$friendly_url = $friendly_url.'-p'.$news_id;
	    	}
	    ...
	    public function deleteNews(){
			...
			$row = $this->getDetail_New('news',array('news_id'), array(&$news_id),'i');
			if($this->Delete_1('news','',array(&$news_id),array('news_id'),'i')){

				echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=tin-tuc&khoa_id='.$row['khoa_id']));
...
// echo json_encode(array('error'=>'Lỗi, mời thử lại check'));die;
   	...
   	/*----------------- Menu -----------------------*/
   		...
   		public function getMenuType(){
  			...
  			$khoa_id = $_GET['khoa_id'] ? $_GET['khoa_id'] : '0';
  			...
  			switch ($menu_type) {
  				...
  				case '5':
  					...
  					$list = $this->getList('newscat','khoa_id',$khoa_id,'newscat_id','asc','','','');
  				...
  				case '7':
  					...
  					$list = $this->getList('news','khoa_id',$khoa_id,'newscat_id','asc','','','');
  				...
  				case '13':
  					...
  					$list = $this->getList('page','khoa_id',$khoa_id,'page_id','asc','','','');
  				...
  				case '17':
  					...
  					$list = $this->getList('khoa','','','id','asc','','','');

  					?>

  						<p class="titleRightNCP">Trang Khoa</p>

		                <select name="khoa_menu_id" id="" class="select2">
  		...
  		public function addMenu(){
  			...
  			$khoa_menu_id = isset($_POST['khoa_menu_id']) ? $_POST['khoa_menu_id'] : '0';


  			if (empty($menu_parent)) {
  				$ko = 0;
  				$menu_top = $this->getDetail_New('menu', array('menu_parent', 'khoa_id'), array(&$ko, &$khoa_id), 'ii');
  				$menu_parent = $menu_top['menu_id'];
  				// echo json_encode(array('error'=>'Lỗi, mời thử lại'.$menu_parent));die;
  			}
  			...
  			if ($insert != '') {
	    		...
	    		foreach ($languages as $key => $lang) {
  				...
  				echo json_encode(array('success'=>'Thêm mới menu thành công', 'url'=>'index.php?page=sua-trinh-don&id='.$insert.'&khoa_id='.$khoa_id));
  		...
  		public function updateMenu(){
            ...
            $service_id = isset($_POST['service_id']) ? $_POST['service_id'] : '';
  			...
  			$row = $this->getDetail_New('menu', array('menu_id'), array(&$menu_id), 'i');
  			if ($row['menu_parent'] == 0) {
  				echo json_encode(array('error'=>'Bạn không được thay đổi mục này'));die;
  			}


  			if ($menu_parent == 0) {
  				echo json_encode(array('error'=>'Bạn không được chọn cấp 1'));die;
  			}
  		...
  		public function deleteMenu(){
			...
			$row = $this->getDetail_New('menu', array('menu_id'), array(&$menu_id), 'i');
			// echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=trinh-don&khoa_id='.$row['khoa_id']));die;
			if($this->Delete_1('menu','',array(&$menu_id),array('menu_id'),'i')){
				

				echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=trinh-don&khoa_id='.$row['khoa_id']));
